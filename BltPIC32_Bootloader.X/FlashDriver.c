...
const uint_t countPerMicroSec = ((SYS_FREQ/1000000)/2);
...
* Note:		 	None.
********************************************************************/
uint_t __attribute__((nomips16)) NVMemOperation(uint_t nvmop)
{
    int	int_status;
    int	susp;
 ...
 ********************************************************************/
uint_t NVMemErasePage(void* address)
{
    uint_t res;
 ...
 ********************************************************************/
uint_t NVMemWriteWord(void* address, uint_t data)
{
    uint_t res;
 ...
 ********************************************************************/
uint_t NVMemWriteRow(void* address, void* data)
{
    unsigned int res;
...
void updateFirmware__Flash(void) {
   uint32_t retNvWrite = 0;
   
    rowOffset    = (rowCnt * BYTE_ROW_SIZE);
    rowAddr      = (APP_FLASH_BASE_ADDRESS + rowOffset);
    
    // Write row to flash
    retNvWrite = NVMemWriteRow((void *)rowAddr, (void *)uart_rxbuffer);
...
void eraseSeq__Flash(void) {
    uint32_t addr2erase = APP_FLASH_BASE_ADDRESS;

    // Erase pages to write
    while(addr2erase < APP_FLASH_END_ADDRES)
    {
        NVMemErasePage((void *) addr2erase);
        addr2erase += BYTE_PAGE_SIZE;
    }
}